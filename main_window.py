...
def format_fps(text: str) -> str:
    """
    Extract numeric fps, drop '/1' or trailing .0,
    convert to superscript + 'ᶠᵖˢ'.
    """
...
class IPTVChecker(QtWidgets.QMainWindow):
    ...
    def start_check(self):
        ...
        # Clear previous
        for k in ("working", "black_screen", "non_working"):
            getattr(self, f"tbl_{k}").setRowCount(0)
        ...
        for _ in range(self.sp_workers.value()):
            ...
            t.finished.connect(self._on_all_done)
            t.start()
    ...
    def _on_result(self, name, status, res, fps):
        ...
        # Table FPS: numeric only
        m_fps = re.search(r"(\d+(?:\.\d+)?)", fps or "")
        fps_num = m_fps.group(1) if m_fps else ''
        if fps_num.endswith('.0'):
            fps_num = fps_num[:-2]
        display_fps = fps_num or '–'

        # Insert into row
        for col, value in enumerate([formatted_name, res, display_fps]):
            tbl.setItem(row, col, QtWidgets.QTableWidgetItem(value))
