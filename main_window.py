...
class IPTVChecker(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.status_map = {}       # uid -> status string ('UP','BLACK_SCREEN','NON_WORKING')
        self.threads = []
    ...
    def _build_ui(self):
        ...
        # switcher bar
        bar = QtWidgets.QFrame(); bar.setFixedHeight(40); bar.setStyleSheet("background-color: #5b2fc9;")
        ...
        btn_options = QtWidgets.QPushButton("Options"); btn_options.setStyleSheet("color:white; background:transparent; border:none;")
        btn_options.clicked.connect(self._open_options); bar_layout.addWidget(btn_options)
        bar_layout.addStretch(); main_v.addWidget(bar)
        ...
        hdr0=QtWidgets.QLabel("DonTV IPTV Checker"); hdr0.setAlignment(QtCore.Qt.AlignCenter); pv0.addWidget(hdr0)
        # control buttons
        ...
        for status in ("working","black_screen","non_working"):
            ...
            tbl.setHorizontalHeaderLabels(headers); tbl.horizontalHeader().setStretchLastSection(True)
            tbl.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers);
            tbl.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
            QtWidgets.QVBoxLayout(grp).addWidget(tbl)
            setattr(self, f"tbl_{status}", tbl); panes.addWidget(grp)
        pv0.addLayout(panes)
        # console
        console_grp=QtWidgets.QGroupBox("Console"); cv=QtWidgets.QVBoxLayout(console_grp)
        flt_h=QtWidgets.QHBoxLayout()
        self.cb_show_working=QtWidgets.QCheckBox("Show Working"); self.cb_show_working.setChecked(True); self.cb_show_working.stateChanged.connect(self._refresh_console)
        self.cb_show_info=QtWidgets.QCheckBox("Show Info"); self.cb_show_info.setChecked(True); self.cb_show_info.stateChanged.connect(self._refresh_console)
        self.cb_show_error=QtWidgets.QCheckBox("Show Error"); self.cb_show_error.setChecked(True); self.cb_show_error.stateChanged.connect(self._refresh_console)
        for cb in (self.cb_show_working,self.cb_show_info,self.cb_show_error): flt_h.addWidget(cb)
        cv.addLayout(flt_h);
        self.te_console=QtWidgets.QTextEdit(); self.te_console.setReadOnly(True); cv.addWidget(self.te_console)
        pv0.addWidget(console_grp); self.pages.addWidget(page0)
        # page1: playlist sorter placeholder
        page1=QtWidgets.QWidget(); pv1=QtWidgets.QVBoxLayout(page1)
        pv1.setContentsMargins(10,10,10,10); pv1.setSpacing(20)
        hdr1=QtWidgets.QLabel("DonTV Playlist Sorter"); hdr1.setAlignment(QtCore.Qt.AlignCenter); pv1.addWidget(hdr1)
        placeholder=QtWidgets.QLabel("Playlist Sorter functionality coming soon."); placeholder.setAlignment(QtCore.Qt.AlignCenter); pv1.addWidget(placeholder)
    ...
    def _open_options(self):
        ...
        dlg.sp_workers.setValue(self.workers); dlg.sp_retries.setValue(self.retries)
        dlg.sp_timeout.setValue(self.timeout); dlg.cb_split.setChecked(self.split)
        dlg.cb_update_quality.setChecked(self.update_quality); dlg.cb_update_fps.setChecked(self.update_fps)
        dlg.cb_include_untested.setChecked(self.include_untested); dlg.le_out.setText(self.output_dir)
        if self.m3u_file:
        ...
        result=dlg.exec_(); self.setStyleSheet(STYLE_SHEET)
    ...
    def start_check(self):
        if not self.m3u_file or not self.selected_groups:
            ...
            return
        with open(self.m3u_file,'r',encoding='utf-8') as f: self.original_lines=f.readlines()
        self.group_entries,self.categories=parse_groups(self.m3u_file)
        for s in ('working','black_screen','non_working'): getattr(self,f"tbl_{s}").setRowCount(0)
        self.log_records.clear(); self.te_console.clear(); self.entry_map={}; self.status_map={}
        # build entry_map
        for grp in self.selected_groups:
            for e in self.group_entries.get(grp,[]):
                self.entry_map[e['uid']]=e
                self.status_map[e['uid']]='UNTESTED'
        q=queue.Queue()
        for uid,e in self.entry_map.items(): q.put(e.copy())
        self.tasks_q=q; self.status.showMessage(f"Queued {q.qsize()} tasks from {len(self.selected_groups)} groups",3000)
        # start workers
        self.threads=[]
    ...
    def _on_result(self, entry, status, res, fps):
        uid=entry['uid']; self.status_map[uid]=status
        ...
        item=QtWidgets.QTableWidgetItem(display); item.setData(QtCore.Qt.UserRole,uid)
    ...
    def _on_log(self, level, msg):
        color=CONSOLE_COLORS.get(level,'black'); self.log_records.append((level,msg,color)); self._refresh_console()

    def _refresh_console(self):
        self.te_console.clear(); cursor=self.te_console.textCursor()
        for lvl,m,color in self.log_records:
            show=getattr(self,f"cb_show_{lvl}",None)
            if show and show.isChecked():
                cursor.movePosition(QtGui.QTextCursor.End); cursor.insertHtml(f'<span style="color:{color}">{m}</span><br>')
    ...
    def _monitor_threads(self):
        if all(not t.isRunning() for t in self.threads):
            self._poll_timer.stop(); QtCore.QTimer.singleShot(0,self._finish_and_write)

    def _finish_and_write(self):
        # write outputs using external module
