...
# Regexes to rewrite EXTINF lines
...
class IPTVChecker(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        c_v = QtWidgets.QVBoxLayout(console_grp)
        f_h = QtWidgets.QHBoxLayout()
    ...
    def start_check(self):
        ...
        # Queue entries for selected groups
    ...
    def _on_result(self, entry, status, res, fps):
        ...
        if status=='UP':
            if self.update_quality:
                q = resolution_to_label(res); display += f" {q}" if q else ""
            if self.update_fps:
                f_lbl = format_fps(fps); display += f" {f_lbl}" if f_lbl else ""
    ...
    def _refresh_console(self):
        ...
        for lvl,msg in self.log_records:
            if show.get(lvl):
                self.te_console.append(f'<span style="color:{cols[lvl]}">{msg}</span>')

    def _toggle_pause(self):
        self._is_paused = not self._is_paused
        for t in self.threads:
            if self._is_paused: t.pause()
            else:               t.resume()
        ...
        def write(fn,uids):
            with open(fn,'w',encoding='utf-8') as f:
                ...
                for uid in uids:
                    ...
                    if uid in tested:
                        extinf = extinf_tvg_re.sub(lambda m: f'{m.group(1)}{disp_map[uid]}{m.group(2)}', extinf)
                        extinf = extinf_comma_re.sub(lambda m: f'{m.group(1)}{disp_map[uid]}', extinf)
                    f.write(extinf + '\n')
                    f.write(url    + '\n')
            paths.append(fn)
